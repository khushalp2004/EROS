...
function AppContent() {
  ...
  // Show notifications for authority and admin users (but different types)
  const shouldShowNotifications = isAuthenticated && (user?.role === 'authority' || user?.role === 'admin');
  ...
  return (
    <NotificationProvider userId={1}> {/* Default user ID for demo */}
      <Router>
        ...
        {/* Toast Notifications - Only show for admin users */}
        {shouldShowNotifications && <ToastContainer />}
